...
def get_imagenet_val_dataset(transform, root_dir):
    ...
    dataset = ImageNetValDataset(img_dir, val_label_file, val_class_index_file, transform)
    return dataset


def save_params_csv(params, filepath='params.csv'):
    """Saves the best parameters to a CSV file after optimization.

    Args:
        params (dict): A dictionary containing the parameter names and values.
        filepath (str, optional): The path to the CSV file. Defaults to 'params.csv'.
    """
    try:
        with open(filepath, 'w', newline='') as csvfile:
            ...
            # Write parameter name and value rows
            for key, value in params.items():
                writer.writerow([key, value])
        print(f"Best parameters saved to: {filepath}")
    except Exception as e:
        print(f"Error saving best parameters to CSV: {e}")


def save_pickle_data(data, filepath='data.pkl'):
    """Saves the data to a pickle file.

    Args:
        data: A data variable to pickle.
        filepath (str, optional): The path to the pickle file. Defaults to 'data.pkl'.
    """
    try:
...
def load_pickle_data(filepath='data.pkl'):
    ...
    Returns:
        Any: The loaded data, or None if the file is not found or an error occurs.
    """
    try:
        if os.path.exists(filepath):
            ...
            print(f"Warning: Pickle file not found at {filepath}")
            return None
    except Exception as e:
