...
# --- OpenXAI Imports ---
from openxai.explainers import (
    BaseExplainer,
    Gradient,
    SmoothGrad,
    InputTimesGradient,
...
from openxai.explainers.catalog import LRP

# --- Captum Normalization ---
...
# --- Helper Imports ---
try:
    from tutorials.img.helpers import (
        get_torchvision_model,
        get_imagenet_val_dataset,
        denormalize_image,
        save_pickle_data,
        load_pickle_data,
    )
...
# --- Evaluation Function using OpenXAI ---
def evaluate(args):
    # setup
    cwd = os.getcwd()
    savedir_base = os.path.join(cwd, f"results/openxai/{args.model}")
    ...
    dataset = get_imagenet_val_dataset(transform, args.data_dir)
    ...
    # lime_data = torch.stack([datum for datum, _ in dataset])
    # --- Define Open Explainers ---

    openxai_explainers = {
        "gradient": Gradient(model),
        "gradient_x_input": InputTimesGradient(model),
        ...
        "kernel_shap": SHAPExplainerC(model),  # Uncomment if needed (SLOW)
        # "lime": LIME(model, lime_data),  # Uncomment if needed (SLOW, requires more setup)
        "integrated_gradients": IntegratedGradients(model),  # If needed
    }

    # Filter out None explainers
    active_explainers: Dict[Dict[str, BaseExplainer]] = defaultdict(dict)
    for explainer in args.eval_explainer:
        if explainer in openxai_explainers:
            print(f"Evaluating explainer: {explainer}")
            active_explainers[explainer] = openxai_explainers[explainer]
        else:
            print(
                f"Requested explainer '{explainer}' not available or inactive. Evaluating all active ones."
            )
    ...
    try:
        ...
        abpc_metric = AbPC(model=model)

        # simplicity_metric = Simplicity(model=model)
        complexity_metric = Complexity(model=model)

        composite_metric = CompositeMetrics(model=model)
        composite_metric.composite_metrics = [
            AbPC(model=model),
            Simplicity(model=model),
        ]
    ...
    # --- Evaluation Loop ---
    print("\n--- Starting OpenXAI Evaluation ---")
    results = defaultdict(
        lambda: defaultdict(list)
    )  # results[explainer][metric] = [scores]
    ...
    for batch_idx, batch_data in enumerate(tqdm(dataloader, desc="Evaluating Batches")):
        ...
        for explainer_name, explainer_instance in active_explainers.items():
            ...
            tqdm.write(
                f"  Batch {batch_idx+1}/{len(dataloader)}, Explainer: {explainer_name}"
            )

            # --- Compute Attributions ---
            current_attribution = explainer_instance.get_explanations(
                inputs, pred_label_idx
            )

            if current_attribution is None:
                tqdm.write(
                    f"    Skipping metrics for {explainer_name} (attribution failed or type not handled)"
                )
                continue

            # --- Postprocessing ---
            pp_attribution = normalize_attr(
                current_attribution.detach().cpu().numpy(),
                sign="absolute_value",
                reduction_axis=1,
            )
            ...
            # plt.tight_layout()
            # plt.savefig(f'openxai_attrs_debug/openxai_{explainer_name}.jpg')
            # plt.close()
            # continue
            ...
            for metric_name, metric_instance in metrics_to_run.items():
                if metric_name == "complexity":
                    score = metric_instance.evaluate(
                        inputs=inputs,
                        attributions=pp_attribution,
                        targets=pred_label_idx,
                    )
                    print(f'complexity score: {score}')
                elif (
                    metric_name == "ab_pc" or metric_name == "simplicity"
                ):  # Assuming Simplicity is like AbPC
                    score = metric_instance.evaluate(
                        inputs=inputs,
                        attributions=pp_attribution,
                        targets=pred_label_idx,
                    ...
                    continue
                # print(results[explainer_name], score)
...
def visualize(args):
    ...
    cwd = os.getcwd()
    datadir = os.path.join(cwd, f"results/openxai/{args.model}")
    ...
    for explainer_key in args.eval_explainer:
        eval_results[explainer_key] = load_pickle_data(
            os.path.join(datadir, f"evals/{explainer_key}.pkl")
        )
        ...
        for target_metric_key in TARGET_METRIC_KEYS:
            bar_means[target_metric_key].append(
                np.array(eval_results[explainer_key][target_metric_key]).mean()
            )
            bar_stds[target_metric_key].append(
                np.array(eval_results[explainer_key][target_metric_key]).std()
            )
    ...
    for target_metric_key in TARGET_METRIC_KEYS:
        ...
        plt.bar(
            r,
            bar_means[target_metric_key],
            yerr=bar_stds[target_metric_key],
            width=bar_width,
            edgecolor="grey",
            color=colors,
            capsize=5,
        ...
        plt.xticks(
            [r_ for r_ in r], args.eval_explainer, rotation=30, ha="right", fontsize=10
        )
        ...
        savedir_quant = os.path.join(
            datadir, f"evals/figures/opt_evals_{target_metric_key}.jpg"
        )
        os.makedirs(os.path.dirname(savedir_quant), exist_ok=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate XAI methods using OpenXAI")
    parser.add_argument(
        "--model",
        type=str,
        choices=TORCHVISION_MODEL_CHOICES,
        required=True,
    ...
    parser.add_argument(
        ...
        default=None,
        nargs="+",
        help="Evaluate only a specific explainer by name",
    )
