...
class muFidelity(EvaluatorMetric):
    ...
    def __call__(self, model: Model, explainer: Explainer, sample: Tensor, label, pred, pred_idx, result):
        ...
        subset_mask = torch.randn((sample_size[0], self.grid_size ** 2)).to(device)
        subset_mask = torch.argsort(subset_mask, dim=-1) > subset_size
        subset_mask = torch.reshape(subset_mask.type(torch.float32), (sample_size[0], 1, self.grid_size, self.grid_size))
        subset_mask = transforms.Resize(perturbed_sample.shape[-2:], transforms.InterpolationMode("nearest")).forward(subset_mask)

        # Use the masks to set the selected subsets to baseline state
        masked_sample = perturbed_sample * subset_mask + (1.0 - subset_mask) * self.baseline
        ...
        attrs = attrs.sum(dim=(1,2,3))
        corr, p = spearmanr(masked_pred.cpu().detach().numpy(), attrs.cpu().detach().numpy())
