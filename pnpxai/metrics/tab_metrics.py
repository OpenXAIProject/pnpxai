...
from pnpxai.core._types import XGBModel, SklearnModel, TorchModel

class TorchWrapperModel(nn.Module):
    def __init__(self, model):
        super().__init__()
        self.inner_model = model
    
    def forward(self, x):
        return tensor(self.inner_model.predict_proba(x.cpu().numpy())).float().log_softmax(dim=1)
...
class TabABPC(Metric):
    ...
    def evaluate(self, 
        ...
        if isinstance(inputs, Tensor):
            ...
            inputs = np.array([i.detach().cpu().numpy()[0] for i in inputs])
        ...
        if isinstance(self.model, Union[XGBModel, SklearnModel]):
            model = sklearn_wrapping(self.model)
        else:
            model = self.model
...
class TabAvgSensitivity(Metric):
    def __init__(self, model, **kwargs):
        self.model = model
        super().__init__(model, **kwargs)
    ...
    def evaluate(self, 
        ...
        if perturb_func is not None:
            metric_kwargs["perturb_func"] = perturb_func

        self.metric = qt.AvgSensitivity(**metric_kwargs)
...
class TabInfidelity(Metric):
    ...
    def evaluate(self, 
                ...
                explain_func: Callable,
                loss_func: str = "mse",
                perturb_patch_sizes: list = [1],
                n_perturb_samples: int = 10,
                perturb_baseline: int = 0,
                perturb_func: Callable = None,
                **kwargs
                ) -> Tensor:
        ...
        metric_kwargs = {
            "loss_func" : loss_func,
            "perturb_patch_sizes" : perturb_patch_sizes,
        ...
        self.metric = Infidelity(**metric_kwargs)
    ...
    def evaluate_instance(
        ...
        for _ in range(self.n_perturb_samples):
            ...
            for patch_size in self.perturb_patch_sizes:
                # PNPXAI
                if a.ndim == 1:
                    pred_deltas = np.zeros(a.shape[0])
                    a_sums = np.zeros(a.shape[0])
                else:
                ...
                for i_x, top_left_x in enumerate(range(0, x1, patch_size)):
                    for i_y, top_left_y in enumerate(range(0, x2, patch_size)):
                        ...
                        # PNPXAI
                        if x.ndim == 1:
                            pred_deltas[i_x] = y_pred - y_pred_perturb
                            a_sums[i_x] = np.sum(a_diff)
                        else:
